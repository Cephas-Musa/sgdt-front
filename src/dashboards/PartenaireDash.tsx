...
export default function PartenaireDash() {
  ...
  if (!partenaire)
    return <div className="p-8 text-center text-muted-foreground">Partenaire non trouvé.</div>;
    ...
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={`$${stats.commissionMoyenne}`}
          hint="Par dossier"
        />
        <StatCard
          icon={Calendar}
          label="Dernier dossier"
          value={stats.dernierDossier}
          hint="Date du dernier traitement"
        />
      </div>
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground mr-2">
                      {pb.commissions.length} type{pb.commissions.length > 1 ? "s" : ""}
                    </span>
                    {isExpanded ? (
              ...
              <tfoot>
                <tr className="border-t-2 border-border font-semibold bg-muted/30">
                  ...
                  <td className="px-3 py-2 text-right">
                    ${pageTx.reduce((s, t) => s + t.prixGlobal, 0)}
                  </td>
                  <td className="px-3 py-2 text-right text-success">
                    ${pageTx.reduce((s, t) => s + t.partPartenaire, 0)}
                  </td>
                  <td className="px-3 py-2 text-right">
                    ${pageTx.reduce((s, t) => s + t.partSysteme, 0)}
                  </td>
                </tr>
              </tfoot>
            </table>
