  ...
  return (
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={DollarSign} label="Solde disponible" value={`$${solde}`} hint="Vérifier avant impression" />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Panel
          ...
          actions={
            <FormDialog
              trigger={<Button><Plus className="mr-1.5 h-4 w-4" />Nouveau manifest</Button>}
              title="Créer un Empty Manifest"
              onSubmit={() => {
                if (solde < 25) {
            ...
            >
              ...
              <div className={`mb-4 rounded-md p-3 border flex gap-3 ${solde >= 25 ? "bg-success/10 border-success/30" : "bg-destructive/10 border-destructive/30"}`}>
                <DollarSign className={`h-5 w-5 mt-0.5 ${solde >= 25 ? "text-success" : "text-destructive"}`} />
                <div className="text-sm">
                  <p className="font-medium">Solde: ${solde}</p>
                  <p className="text-muted-foreground">{solde >= 25 ? "Solde suffisant pour créer un manifest" : "⚠ Solde insuffisant — rechargez votre compte"}</p>
                </div>
              </div>

              <FormGrid>
                <Field label="Référence" required><Input placeholder="EMP/2025/…" /></Field>
                ...
                <Field label="Type de véhicule" required><Input placeholder="Camion, Pick-up…" /></Field>
                <Field label="Transporteur" required><Input /></Field>
                <Field label="Lieu de chargement" required><Input /></Field>
                <Field label="Propriétaire véhicule"><Input /></Field>
        ...
        >
          <div className="py-4 text-center text-sm text-muted-foreground">
            Cliquez sur "Nouveau manifest" pour créer un document. Le solde sera vérifié automatiquement avant l'attribution du numéro.
          </div>
        </Panel>
              ...
              <tbody>
                {EMPTY_MANIFESTS.slice(0, 8).map((m) => (
                  <tr key={m.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-3 py-2">
                      <span className={`rounded-full px-2 py-0.5 text-xs ${m.status === "payé" ? "bg-success/15 text-success" : "bg-warning/15 text-warning"}`}>{m.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
