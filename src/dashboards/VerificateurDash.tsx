  ...
  return (
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={Search} label="En vérification" value={DOSSIERS.filter(d => d.status === "en_cours").length} />
        <StatCard icon={FileText} label="Apurés" value={DOSSIERS.filter(d => d.status === "apure").length} />
      </div>
          ...
          <TabsContent value="dossiers" className="mt-4 space-y-4">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                />
                <Button variant="outline" onClick={() => { setSearchRef(""); setSearchYear(""); }}>
                  Réinitialiser
                </Button>
              </div>
            </div>
                        ...
                        <td className="px-3 py-2 text-right">
                          ...
                          >
                            ...
                            <FormGrid>
                              ...
                              <Field label="Référence DRA" required><Input value={d.dra} /></Field>
                              <Field label="Date" required><Input type="date" /></Field>
                              <Field label="Référence T1" required><Input value={d.t1} /></Field>
                              <Field label="Date T1"><Input type="date" /></Field>
                              <Field label="Information page info"><Input /></Field>
                              <Field label="Quantité totale" required><Input type="number" /></Field>
                              <Field label="Information"><Input /></Field>
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Input
                  ...
                  className="max-w-[180px]"
                />
                <Button variant="outline" onClick={() => { setSearchDRA(""); setDateDebut(""); setDateFin(""); }}>
                  Réinitialiser
                </Button>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {traites.map((d) => (
                      <tr key={d.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                        <td className="px-3 py-2">
                          <Link to="/app/dossiers/$dossierId" params={{ dossierId: d.dossierId }} className="text-accent hover:underline font-mono text-xs">{d.reference}</Link>
                        </td>
                        <td className="px-3 py-2">{d.importateur}</td>
                        <td className="px-3 py-2 capitalize">{d.type}</td>
                        ...
                        <td className="px-3 py-2">
                          <span className={`rounded-full px-2 py-0.5 text-xs ${d.resultat === "conforme" ? "bg-success/15 text-success" : d.resultat === "non_conforme" ? "bg-destructive/15 text-destructive" : "bg-warning/15 text-warning"}`}>{d.resultat.replace("_", " ")}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
