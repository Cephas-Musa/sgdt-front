...
export function SecretairesModule() {
  ...
  return (
    <Panel title="Gestion des Secrétaires" actions={
      <FormDialog
      ...
      >
        ...
        <div className="mt-3 space-y-2">
          <Field label="Mot de passe">
            <div className="flex gap-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => setGenPwd(generatePassword())}>
                Générer
              </Button>
            </div>
          </Field>
        ...
        <div className="mt-3 flex gap-2">
          <Button type="button" variant="outline" size="sm" onClick={() => {
            ...
            <Copy className="mr-1 h-3.5 w-3.5" />Copier identifiants
          </Button>
        </div>
      </FormDialog>
    }>
      <div className="mb-3 flex items-center gap-2 rounded-md bg-accent/5 border border-accent/20 p-3 text-sm">
        ...
        <span>Vous êtes le seul à pouvoir créer et gérer vos secrétaires. Chaque secrétaire vous est rattaché.</span>
      </div>
              ...
              <tr key={s.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(s.motDePasse); toast.info("Mot de passe copié"); }}
                ...
                <td className="px-3 py-2">
                  <span className={`rounded-full px-2 py-0.5 text-xs ${s.status === "actif" ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>
                    {s.status}
                  </span>
                </td>
                <td className="px-3 py-2 text-xs">{s.dossiersTraites}/{s.dossiersAssignes}</td>
