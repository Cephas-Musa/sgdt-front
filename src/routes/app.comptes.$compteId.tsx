...
import { ArrowLeft, User, Phone, Building2, Calendar, Shield, Edit, KeyRound, Ban, CheckCircle2 } from "lucide-react";
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-lg border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className={`rounded-full px-3 py-1 text-sm font-medium ${compte.status === "actif" ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>
                {compte.status}
              </span>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground uppercase tracking-wide">Identifiant</div>
                <div className="font-mono text-sm">{compte.username}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground uppercase tracking-wide">Matricule</div>
              ...
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Shield className="h-3 w-3" /> Rôle</div>
                <div className="text-sm font-medium">{roleLabel}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Building2 className="h-3 w-3" /> Bureau</div>
                <div className="text-sm">{compte.bureau ?? "—"}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground uppercase tracking-wide">Province</div>
              ...
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground uppercase tracking-wide">Créé par</div>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <div className="space-y-2">
              ...
              >
                <FormGrid>
                  <Field label="Nom complet"><Input defaultValue={compte.fullName} /></Field>
                  <Field label="Téléphone"><Input defaultValue={compte.phone} /></Field>
                  <Field label="Bureau"><Input defaultValue={compte.bureau} /></Field>
                  <Field label="Province"><Input defaultValue={compte.province} /></Field>
                </FormGrid>
              </FormDialog>
              ...
              <FormDialog
                ...
                onSubmit={() => toast.success("Mot de passe réinitialisé. Nouveau mot de passe envoyé.")}
              >
                <Field label="Nouveau mot de passe">
                  <div className="flex gap-2">
                    <Input placeholder="Généré automatiquement" readOnly />
                    <Button type="button" variant="outline" size="sm" onClick={() => toast.info("Mot de passe généré")}>
                      <KeyRound className="mr-1 h-3.5 w-3.5" />Générer
                    </Button>
                  </div>
                </Field>
              </FormDialog>
              ...
              {compte.status === "actif" ? (
                ...
                >
                  <CheckCircle2 className="mr-2 h-4 w-4" />Réactiver le compte
                </Button>
              )}
            </div>
          </div>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{compte.dateCreation} par {compte.creePar ?? "—"}</div>
                </div>
              </div>
              <div className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div className={`h-3 w-3 rounded-full ${compte.status === "actif" ? "bg-success" : "bg-destructive"}`} />
                </div>
                <div>
                  <div className="font-medium">{compte.status === "actif" ? "Actif" : "Désactivé"}</div>
