...
const vehicules = [
  { id: 1, plaque: "AA 1001 XY", position: "Barrière entrée", status: "en_attente", depuis: "2h30" },
  ...
  { id: 4, plaque: "AB 1019 XY", position: "Barrière entrée", status: "en_transit", depuis: "30min" },
...
const posColor = (p: string) =>
  p.includes("Barrière") ? "bg-warning/15 text-warning"
  : p.includes("Entrepôt") ? "bg-info/15 text-info"
  : p === "En route" ? "bg-accent/15 text-accent"
  : "bg-muted text-muted-foreground";

const statColor = (s: string) =>
  s === "en_transit" ? "bg-accent/15 text-accent"
  : s === "chargement" || s === "dechargement" ? "bg-success/15 text-success"
  : s === "en_attente" ? "bg-warning/15 text-warning"
  : "bg-muted text-muted-foreground";
...
function LocalisationPage() {
  ...
  const autres = vehicules.filter(v => !v.position.includes("Barrière") && !v.position.includes("Entrepôt")).length;

  return (
    <div>
      <PageHeader title="Localisation véhicules" description="Position actuelle des véhicules en transit et en stationnement" />
      ...
      <div className="mb-6 rounded-lg border border-border bg-card p-4">
        <h3 className="font-medium mb-3 flex items-center gap-2"><Navigation className="h-4 w-4 text-accent" />Flux des véhicules</h3>
        <div className="flex items-center justify-center gap-2 flex-wrap text-sm">
          ...
          <div className="rounded-lg bg-success/10 border border-success/30 px-4 py-3 text-center">
            <div className="font-bold text-lg">{DOSSIERS.filter(d => d.status === "apure").length}</div>
              ...
              {vehicules.map(v => (
                <tr key={v.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-3 py-2">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${posColor(v.position)}`}>{v.position}</span>
                  </td>
                  <td className="px-3 py-2">
                    <span className={`rounded-full px-2 py-0.5 text-xs capitalize ${statColor(v.status)}`}>{v.status.replace("_", " ")}</span>
                  </td>
